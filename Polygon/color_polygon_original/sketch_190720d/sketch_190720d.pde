...
void draw() {
  ...
  for (int i=1; i<=num; i++) {  //i=圈的个数
    ...
    deg = (millis()+136*i)/8.0;  //第一个参数：调整一个循环的大小；第二个参数：调整循环速度的大小
    rad = radians(deg);
    pingPong = sin(rad);
    //不同节奏的来回往复用在笔触上
    ...
    drawPolygon(width/2,height/2,i*16,5);
    //drawPolygon1(i*16, 5);
  }
}

//画多边形方法1 cx, cy=中心坐标 r=点到中心的距离 count=点数/边数
