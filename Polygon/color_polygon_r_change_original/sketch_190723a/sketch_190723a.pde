...
void draw() {
  ...
  for (int i=1; i<=num; i++) {  //i=圈的个数
    ...
    drawDancingPolygon(width/2, height/2, i*12, 5);

    //pushMatrix();
    //translate(width/2, height/2);
    //rotate(radians(s));
    //drawDancingPolygon(width/2,height/2,i*11, 5);
    //popMatrix();
  }
}
...
void drawDancingPolygon(int cx, int cy, 
  float r, int count) {
  float unit;
  float mr;
  float x;
  ...
  for (int i=1; i <=count; i++) {
    unit = 360/count;
    mr = r+map(sin(radians((millis()+46*i)/3.4)),-1,1,-30,30); 
...
void drawPolygon1(float r, int count) {
  ...
  for (int i=1; i <=count; i++) {
    ...
    float mr = r+map(sin(radians(millis()/3.4+461*i)),-1,1,-30,30); 
